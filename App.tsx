    ...
    const checkAuthAndFetchData = async () => {
      ...
      if (token) {
        try {
          ...
          if (brand.status === 'fulfilled' && brand.value) setSavedBrand(brand.value);
  ...
  const renderContent = () => {
    switch (currentView) {
      ...
      case AppView.CONTENT_GENERATOR:
        return <ContentGenerator history={contentHistory} onAddToHistory={handleAddContent} brand={savedBrand} />;
      ...
      case AppView.SETTINGS:
        return <Settings user={user} userStats={userStats} onLogout={handleLogout} />;
      default:
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row">
      ...
      <div className="md:hidden bg-white border-b border-gray-200 p-4 flex justify-between items-center sticky top-0 z-20">
        ...
        <div className="flex items-center gap-4">
          {cartItems.length > 0 && (
            <button onClick={() => handleNavigate(AppView.CART)} className="relative text-xl">
              🛒
          ...
          <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="text-gray-600 focus:outline-none text-2xl">
            {isMenuOpen ? '✕' : '☰'}
          </button>
        </div>
      </div>
      ...
      `}>
        <div className="p-6 border-b border-gray-100 hidden md:flex items-center space-x-2 cursor-pointer" onClick={() => handleNavigate(AppView.DASHBOARD)}>
          <div className="w-8 h-8 bg-green-600 rounded-lg flex items-center justify-center text-white font-bold">S</div>
        ...
        <div className="px-6 pt-6 pb-2">
          <p className="text-xs font-semibold text-gray-400 uppercase">Business</p>
          <p className="font-bold text-gray-800 truncate">{user.businessName}</p>
        </div>
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          ...
          <div className="pt-4 pb-2">
            <p className="px-3 text-xs font-semibold text-gray-400 uppercase">Growth</p>
          </div>
          ...
          <div className="pt-4 pb-2">
            <p className="px-3 text-xs font-semibold text-gray-400 uppercase">Help</p>
          </div>
        ...
        <div className="p-4 border-t border-gray-100">
          <div className="bg-gradient-to-r from-indigo-600 to-purple-600 p-4 rounded-lg text-white text-center">
            <p className="text-xs font-medium opacity-90 mb-2">Upgrade to Smart Access</p>
            <p className="text-xs opacity-75 mb-3">Get unlimited AI & Pro guides</p>
            <button
